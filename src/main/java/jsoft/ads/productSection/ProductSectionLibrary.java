...
public class ProductSectionLibrary {
	public static ArrayList<String> viewProductSection(
			ArrayList<ProductSectionObject> items, 
			ProductSectionObject productSection, 
			UserObject user,
...
//		tmp.append("<h5 class=\"card-title\">Table with stripped rows</h5>");
		...
		items.forEach(item -> {
			...
			tmp.append("<td>" + item.getPs_created_date() + "</td>");
			tmp.append("<td>" + item.getPs_name() + "</td>");
				
			productGroups.forEach(productGroup -> {
				if(productGroup.getPg_id() == item.getPs_pg_id()) {
...
//			System.out.println("----" + section.isSection_delete() + "----");
			...
			if (productSection.isPs_delete()) {
				tmp.append("<td>" + item.getPs_deleted_date() + "</td>");
				tmp.append(
						"<td class=\"align-middle\"><a href=\"/datn/productSection/dr?id="+item.getPs_id()+"&r\" class=\"btn btn-primary btn-sm\"><i class=\"bi bi-reply\"></i></a></td>");
				tmp.append(
						"<td class=\"align-middle\"><button class=\"btn btn-danger btn-sm\" data-bs-toggle=\"modal\" data-bs-target=\"#delproductGroup"
								+ item.getPs_id() + "\"><i class=\"bi bi-trash\"></i></button></td>");
				tmp.append(ProductSectionLibrary.viewDelProductSection(item));
				tmp.append("<th scope=\"row\">" + item.getPs_id() + "</th>");
			} else {
				tmp.append("<td>" + manager_name.get(item.getPs_manager_id()) + "</td>");
				tmp.append(
						"<td class=\"align-middle\"><a href=\"#\" class=\"btn btn-primary btn-sm\"><i class=\"bi bi-eye\"></i></a></td>");
				tmp.append("<td class=\"align-middle\"><a href=\"/datn/productSection/profiles?id=" + item.getPs_id()
		...
		tmp.append("</div>");
		tmp.append("</div>");
		if(productSection.getPs_name() != null) {
			tmp.append(ProductSectionLibrary.getPaging("/datn/productSection/list?key="+productSection.getPs_name()+"&", page, total, totalPage));
		} else {
			tmp.append(ProductSectionLibrary.getPaging("/datn/productSection/list?", page, total, totalPage));			
		}
		view.add(tmp.toString());

		// view 1
		view.add(ProductSectionLibrary.viewManagerOption(datas, productSection.getPs_manager_id()));
		
		// danh sach nhom san pham 2
		view.add(ProductSectionLibrary.viewProductGroupOption(productGroups, productSection.getPs_pg_id()));

		return view;
	}

	public static StringBuilder viewDelProductSection(ProductSectionObject item) {
		...
		tmp.append("<div class=\"modal fade\" id=\"delproductGroup" + item.getPs_id()
				+ "\" tabindex=\"-1\" aria-labelledby=\"productGroupLabel" + item.getPs_id()
				+ "\" aria-hidden=\"true\">");
		tmp.append("<div class=\"modal-dialog modal-lg\">");
		tmp.append("<div class=\"modal-content\">");
		tmp.append("<div class=\"modal-header\">");
		tmp.append("<h1 class=\"modal-title fs-5\" id=\"productGroupLabel" + item.getPs_id() + "\">" + title + "</h1>");
		tmp.append(
		...
		if (item.isPs_delete()) {
			tmp.append("Bạn sẽ xóa vĩnh viễn thành phần <b>").append(item.getPs_name())
					.append("</b> <br>");
			tmp.append("Thành phần không thể phục hồi được nữa.");
		} else {
			tmp.append("Bạn có chắc chắn xóa tài khoản <b>").append(item.getPs_name())
					.append("</b> <br>");
		...
		if (item.isPs_delete()) {
			tmp.append("<a href=\"/datn/productSection/dr?id=" + item.getPs_id() + "&pid="
					+ item.getPs_created_author_id() + "\" class=\"btn btn-danger\">Xóa vĩnh viễn</a>");
		} else {
			tmp.append("<a href=\"/datn/productSection/dr?id=" + item.getPs_id() + "&t&pid="
					+ item.getPs_created_author_id() + "\" class=\"btn btn-danger\">Xóa</a>"); // dr = delete +
																									// restore
		}
...
//			tmp.append("aaaaaa");
	...
	public static String viewProductGroupOption(ArrayList<ProductGroupObject> productGroups, int selectedId) {
		StringBuilder tmp = new StringBuilder();
