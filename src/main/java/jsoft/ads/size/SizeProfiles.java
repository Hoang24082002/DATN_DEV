...
/**
 * Servlet implementation class SizeProfiles
 */
@WebServlet("/size/profiles")
public class SizeProfiles extends HttpServlet {
	...
	public void view(HttpServletRequest request, HttpServletResponse response, UserObject user)
		...
		if (e_s != null) {
			s_name = e_s.getS_name();
			s_notes = e_s.getS_notes();
			s_manager_id = e_s.getS_manager_id();
		...
		similar.setS_manager_id(s_manager_id);
		...
		out.append("<h1>Thông tin kích cỡ sản phẩm</h1>");
		...
		out.append("<form method=\"post\" action=\"/datn/size/profiles\">");
		...
		out.append("<input name=\"txtSizeName\" type=\"text\" class=\"form-control\" id=\"ps_name\" value=\""+ s_name + "\">");
		out.append("</div>");
		out.append("</div>");
		...
		out.append(
				"<textarea name=\"txtSizeNotes\" class=\"form-control\" id=\"ps_notes\" style=\"height: 100px\">"
						+ s_notes + "</textarea>");
		out.append("</div>");
		out.append("</div>");

		out.append("<div class=\"row mb-3 row align-items-center text-end\">");
		out.append("<label for=\"manager\" class=\"col-md-3 col-lg-2 col-form-label\">Người quản lý màu sắc</label>");
...
//		out.append("</div>"); // end card
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (id > 0) {
			// lấy thông tin
			// lấy thông tin
			String s_name = request.getParameter("txtSizeName");
			int manager = jsoft.library.Utilities.getIntParam(request, "slcManager");
			String s_note = request.getParameter("txtSizeNotes");
			...
			if (s_name != null && !s_name.equalsIgnoreCase("") 
				...
				// khởi tạo đối tượng
				SizeObject e_s = new SizeObject();
				
				e_s.setS_id(id);
				e_s.setS_name(jsoft.library.Utilities.encode(s_name));
				e_s.setS_manager_id(manager);
				e_s.setS_notes(jsoft.library.Utilities.encode(s_note));
				e_s.setS_modified_date(jsoft.library.Utilities_date.getDate());
				...
				// thuc hien cap nhat
				boolean result = sc.editSize(e_s, SIZE_EDIT_TYPE.GENERAL);
				...
				if (result) {
					...
					response.sendRedirect("/datn/size/list?err=edit");
				}
			}
		} else {
			// id ko ton tai
			response.sendRedirect("/datn/size/list?err=profiles");
		}
	}
