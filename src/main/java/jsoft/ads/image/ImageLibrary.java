...
public class ImageLibrary {
	public static ArrayList<String> viewImage(ArrayList<ImageObject> items, ImageObject size, UserObject user,
...
//		tmp.append("<h5 class=\"card-title\">Table with stripped rows</h5>");
		...
		items.forEach(item -> {
			...
			tmp.append("<td>" + item.getI_created_date() + "</td>");
			tmp.append("<td>" + item.getI_name() + "</td>");

//			System.out.println("----" + section.isSection_delete() + "----");

			if (size.isI_delete()) {
				tmp.append("<td>" + item.getI_deleted_date() + "</td>");
				tmp.append(
						"<td class=\"align-middle\"><a href=\"/datn/image/dr?id="+item.getI_id()+"&r\" class=\"btn btn-primary btn-sm\"><i class=\"bi bi-reply\"></i></a></td>");
				tmp.append(
						"<td class=\"align-middle\"><button class=\"btn btn-danger btn-sm\" data-bs-toggle=\"modal\" data-bs-target=\"#delproductGroup"
								+ item.getI_id() + "\"><i class=\"bi bi-trash\"></i></button></td>");
				tmp.append(ImageLibrary.viewDelColor(item));
				tmp.append("<th scope=\"row\">" + item.getI_id() + "</th>");
			} else {
				tmp.append("<td>" + manager_name.get(item.getI_manager_id()) + "</td>");
				tmp.append(
						"<td class=\"align-middle\"><a href=\"#\" class=\"btn btn-primary btn-sm\"><i class=\"bi bi-eye\"></i></a></td>");
				tmp.append("<td class=\"align-middle\"><a href=\"/datn/image/profiles?id=" + item.getI_id()
...
//		System.out.println(section.getSection_manager_id());
		view.add(viewManagerOption(datas, size.getI_manager_id()));

		return view;
	}

	public static StringBuilder viewDelColor(ImageObject item) {
		...
		tmp.append("<div class=\"modal fade\" id=\"delproductGroup" + item.getI_id()
				+ "\" tabindex=\"-1\" aria-labelledby=\"productGroupLabel" + item.getI_id()
				+ "\" aria-hidden=\"true\">");
		tmp.append("<div class=\"modal-dialog modal-lg\">");
		tmp.append("<div class=\"modal-content\">");
		tmp.append("<div class=\"modal-header\">");
		tmp.append("<h1 class=\"modal-title fs-5\" id=\"productGroupLabel" + item.getI_id() + "\">" + title + "</h1>");
		tmp.append(
		...
		if (item.isI_delete()) {
			tmp.append("Bạn sẽ xóa vĩnh viễn thành phần <b>").append(item.getI_name())
					.append("</b> <br>");
			tmp.append("Thành phần không thể phục hồi được nữa.");
		} else {
			tmp.append("Bạn có chắc chắn xóa tài khoản <b>").append(item.getI_name())
					.append("</b> <br>");
		...
		if (item.isI_delete()) {
			tmp.append("<a href=\"/datn/image/dr?id=" + item.getI_id() + "&pid="
					+ item.getI_created_author_id() + "\" class=\"btn btn-danger\">Xóa vĩnh viễn</a>");
		} else {
			tmp.append("<a href=\"/datn/image/dr?id=" + item.getI_id() + "&t&pid="
					+ item.getI_created_author_id() + "\" class=\"btn btn-danger\">Xóa</a>"); // dr = delete +
																									// restore
		}
