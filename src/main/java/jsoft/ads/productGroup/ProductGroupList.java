...
public class ProductGroupList extends HttpServlet {
	...
	protected void view(HttpServletRequest request, HttpServletResponse response, UserObject user)
		...
		// lay cau truc
		Quartet<ProductGroupObject, Short, Byte, UserObject> infos = new Quartet<>(similar, (short)page, (byte) 15, user);
...
//		out.append("<div class=\"card-body\">");
		if (trash == null) {
			...
			out.append("<h1 class=\"modal-title fs-5\" id=\"staticBackdropLabel\"><i class=\"bi bi-plus-lg\"></i> Thêm mới nhóm sản phẩm</h1>");
...
//			npg.setPg_enable(("yes".equals(pg_enable)? true : false));
			...
			response.sendRedirect("/datn/productGroup/list?key=" + jsoft.library.Utilities.encode(key));
		}
